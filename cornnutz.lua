...
for rarity in pairs(RarityColors) do
    local rb = makeBtn(rarityStartY, rarity .. ": " .. (EnabledRarities[rarity] and "ON" or "OFF"), function(b)
        ...
        for _, plot in ipairs(plots:GetChildren()) do
            ...
            if label and label.Text and label.Text:match("^(.-)'s Base") == player.Name then
                ...
                if plot:IsA("Model") then
                    centerCF = select(1, plot:GetBoundingBox())
                end
                if not centerCF then
                    centerCF = (baseRootCache and baseRootCache.CFrame) or CFrame.new()
                    if centerCF.Position == Vector3.new() then
                        for _, d in ipairs(plot:GetDescendants()) do
                            if d:IsA("BasePart") then centerCF = d.CFrame; break end
                        end
                    end
                end
                basePosCache = centerCF.Position
                break
            end
        end
    end
...
RunService.Heartbeat:Connect(function()
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") then
            ...
            if adornee then
                -- If near our base, skip (walk-to only; ESP unaffected)
                if baseCenter then
                    local dBase = (adornee.Position - baseCenter).Magnitude
                    if dBase <= IGNORE_RADIUS then
                        -- skip walking to this one
                        goto continue_obj
                    end
                end

                -- Animals by Generation >= threshold
                ...
                -- Lucky Blocks (respect EnabledRarities; bypass threshold)
    ...
    -- Prefer best Animal; else best Lucky Block
        ...
        if podium.Name == "AnimalOverhead" then
            ...
            if rarity and RarityColors[rarity] then
                if AvoidInMachine and isInMachine(podium) then
                    -- filter visible markers in machine
                else
